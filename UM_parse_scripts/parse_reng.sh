...
#05062015 - SP :Changed the egrep for tail for the ingest stats (last line in script) .Last ingest of the day
#01062015 - SP :Changed the egrep statemement for the ingest stats (last line in script) to look for time 21 insstead of 23-no ingest at 23 so no data collected.
...
#zcat $filename  | grep 'INGEST_STATS_SUMMARY' | egrep -e '\[21:??:??' | awk -F'Ingest summary:' '{print $2}'  | sed -e s/[a-zA-Z\(\)]//g  | sed -e 's/, /;/g' > $exportFileName
