...
#05062015 - SP : Replaced the egrep with tail for the ingest stats (last line in script) to pick up last ingest.
#01062015 - SP : Changed the egrep statemement for the ingest stats (last line in script) to look for time 22 insstead of 23-no ingest at 23 so no data collected.
...
#zcat $filename  | grep 'INGEST_STATS_SUMMARY' | egrep -e '\[22:??:??' | awk -F'Ingest summary:' '{print $2}'  | sed -e s/[a-zA-Z\(\)]//g  | sed -e 's/, /;/g' > $exportFileName
zcat $filename  | grep 'INGEST_STATS_SUMMARY' | tail -1 | awk -F'Ingest summary:' '{print $2}'  | sed -e s/[a-zA-Z\(\)]//g  | sed -e 's/, /;/g' > $exportFileName
